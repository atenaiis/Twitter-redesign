module UsersHelper
    def paragraph(text, style)
            ...
            list_item +=
              div(
                ...
                div(paragraph(user.fullname, 'user') +
                  (paragraph(user.comments.last.body, 'comment') if user.comments.any?),
      ...
      def set_lists
        case params[:list]
        when '1'
          @users = current_user.followings.with_attached_avatar.includes(%i[comments])
          @tag = 'FOLLOWING'
        when '2'
          @users = User.fans(current_user).includes([:comments])
          @tag = 'FOLLOWERS'
        else
          @users = User.all.where('id!=?', current_user.id).with_attached_avatar.includes(%i[comments])
          @tag = 'ALL USERS'
        end
      end
